...
for f in ${OUTPUT_DIR}/*.de; do
  echo "Tokenizing $f..."
  ${OUTPUT_DIR}/mosesdecoder/scripts/tokenizer/tokenizer.perl -q -l de -threads 8 < $f > $f.tok
done

for f in ${OUTPUT_DIR}/*.en; do
  echo "Tokenizing $f..."
  ${OUTPUT_DIR}/mosesdecoder/scripts/tokenizer/tokenizer.perl -q -l en -threads 8 < $f > $f.tok
done
...
${OUTPUT_DIR}/subword-nmt/learn_bpe.py -s 50000 < "${OUTPUT_DIR}/train.de.tok" > "${OUTPUT_DIR}/bpe.de.50k"
${OUTPUT_DIR}/subword-nmt/learn_bpe.py -s 50000 < "${OUTPUT_DIR}/train.en.tok" > "${OUTPUT_DIR}/bpe.en.50k"

# Apply BPE
echo "Applying BPE to all tokenized files..."
for f in ${OUTPUT_DIR}/*.de.tok; do
  ${OUTPUT_DIR}/subword-nmt/apply_bpe.py -c "${OUTPUT_DIR}/bpe.de.50k" < $f > "${f}.bpe.50k"
done
for f in ${OUTPUT_DIR}/*.en.tok; do
  ${OUTPUT_DIR}/subword-nmt/apply_bpe.py -c "${OUTPUT_DIR}/bpe.en.50k" < $f > "${f}.bpe.50k"
done

# Create vocabulary for BPE
${OUTPUT_DIR}/subword-nmt/get_vocab.py < "${OUTPUT_DIR}/train.de.tok.bpe.50k" | cut -f1 -d ' ' > "${OUTPUT_DIR}/vocab.de.bpe.50k"
${OUTPUT_DIR}/subword-nmt/get_vocab.py < "${OUTPUT_DIR}/train.en.tok.bpe.50k" | cut -f1 -d ' ' > "${OUTPUT_DIR}/vocab.en.bpe.50k"
